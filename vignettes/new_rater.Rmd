...
the reliability can be estimated and analyzed as in the first [vignette](iotarelr.html). 
...
First, we simulate an "old" study by calculation Iota2 for the example. Second,
we calculate the most likely true category for each coding unit. These assignments
serves us as "true" categories. In practice, this steps has not do be performed in every
...
```{r}
res_new_rater<-check_new_rater(true_values = expected_categoris$expected_category,
...
This is also documented in the value for *Iota* which range between .6735 for
...
some coding units belonging to that category are not part of the corresponding data.
Here, nearly the same number of forgotten average exams is compensated by coding units
from other categories. Thus, in average the number of coding units in the data for
the average exams equals their true number but on the individual level there are some errors.
...
can be used to to individually guide the training process of new raters since 
the *Assignment Error Matrix* and *Iota* provide hints where error occurs and
