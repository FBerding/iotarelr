...
self-efficacy we assume that participants with better exams have a higher 
confidence in their abilities. The following Figure illustrates this relationship.

![introduction](con_andvalues_intro.png){width=100%}
...
four arguments. With `measure_typ` you must decide which measure of reliability 
...
example this is the performance a participant showed in his exam. Here the value for
*Dynamic Iota Index* is about .267. With 
...
But this is considered in the function. In our example with assume a medium
relationship between achievement and confidence.
...
within a specific range around the prediction (Afifi 2020, p. 119). In the 
...
Here the meaning of a correlation chances every .20 units. The value for lower and
upper mean that which a certainty of 95 % the estimated value for *Kendall's Tau*
...
this context mean that the significant test implies the acceptance of the null
...
# References
- Afifi, A. (2020). Practical Multivariate Analysis (6th ed.). Chapman and Hall/
