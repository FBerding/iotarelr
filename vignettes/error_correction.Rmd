...
```
# 1 Introduction
Literature of content analysis presents the estimation of reliability often as
a part of the development phase of a coding scheme. That is, reliability is
estimated and evaluated in order to inform a revision. If the reliability is 
considered to be sufficient the main study starts. Often, the reliability of the codings
of this main study is not further controlled assuming that the reliability estimates
of the development phase hold for the complete main study. Sometimes researchers
communicate about their findings and assign a category to a coding unit by 
agreeing on the relevant category.

With the *Iota Concept* the reliability of a coding scheme can be taken 
...
such as *Percentage Agreement* or *Krippendorff's Alpha*.  
...
kind of observation. This require at least two raters. Second, involving additional
raters provide more information on a coding unit which can improve the assignments.
...
test are used for reducing errors.

In this vignette we would like to continue our example with the exams from 
the first [vignette](iotarelr.html) and we want to shows you how the error 
...
coding units. While the raters agree on the first two exams they disagree partly
on the exams number 3 to 6. In particular, two raters consider exam 3 to be average
while one rater consider this exams to be good. Thus, there seems to be some
kind of errors and it is not clear which category should be assigned to exam 3.
...
`est_expected_categories()` and use the data ratings as data source. The results
are saved in the object expected_categoris.

```{r}
res_iota2<-compute_iota2(data=iotarelr_written_exams[c("Coder A","Coder B","Coder C")],
                         random_starts = 2,
                         trace = FALSE)
expected_categoris<-est_expected_categories(data=iotarelr_written_exams[c("Coder A","Coder B","Coder C")],
                        aem=res_iota2$categorical_level$raw_estimates$assignment_error_matrix)
head(expected_categoris)
...
For the first two rows the probability that these exams are truly poor exams is about
...
For exam number 3 and 4 the probability to be an average exam is about 48.2 %, to
be a good exam is about 36.4 % and to be a poor exam is about 15.2 %. Thus, it is
...
assignment.
- The most likely true category is assigned to a coding unit.
- Errors made by only one raters can be avoided.
- Researchers do not need to discuss every coding unit which is
